...
public class DcProducerAsync: IDcProducer
{
    ...
    public DcProducerAsync(ILogger<DcProducerSync> logger, ActivitySource activitySource, Meter meter)
    {
        _logger = logger;
    ...
    public async Task<DataTypes.Error?> Produce(DcItem item)
    {
        ...
        {
            Headers kHeaders = [];
            foreach (var dch in item.Headers)
            {
